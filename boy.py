...
class Boy:
    ...
    def get_bb(self):
        if self.state_machine.cur_state == self.ATTACK:
            # 공격 모션일 때의 충돌 박스
            if self.face_dir == 1:
                return self.x + 80, self.y - 100, self.x + 180, self.y + 80
            else:
                return self.x - 180, self.y - 100, self.x - 80, self.y + 80
        elif self.state_machine.cur_state == self.SKILL:
            # 스킬 모션일 때의 충돌 박스
            if self.face_dir == 1:
                return self.x + 80, self.y - 100, self.x + 190, self.y + 80
            else:
                return self.x - 190, self.y - 100, self.x - 80, self.y + 80
        else:
    ...
    def handle_collision(self, group, other):
        if group == 'boy:enemy':
            if self.state_machine.cur_state == self.ATTACK or self.state_machine.cur_state == self.SKILL:
                if other.state_machine.cur_state == other.IDLE:
